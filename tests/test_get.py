...
async def test_simple_get():
    client_reader = MockStreamReader("tests/fixtures/get/simple/request.txt")
    client_writer = MockStreamWriter()
    connection_manager = ConnectionManager()
    config = AppConfig(UPSTREAM_HOST="127.0.0.1", UPSTREAM_PORT=3001)

    response_reader = MockStreamReader("tests/fixtures/get/simple/response.txt")
    ...
    with patch("asyncio.open_connection", mock_open_connection):
        ...
        expected_connection = json.load(
            open("tests/fixtures/get/simple/connection.json")
        )
