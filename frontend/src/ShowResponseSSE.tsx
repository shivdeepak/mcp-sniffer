...
const ShowResponseSSE = ({ connection }: { connection: Connection }) => {
    return <div>
        {connection.response.sse_messages && connection.response.sse_messages.length > 0 ? (
            <>
                            ...
                            <tr key={message.start_time}>
                                ...
                                <td>
                                    ...
                                    {message.data != undefined && (
                                        <pre>data: {typeof message.data === 'string' ? message.data : JSON.stringify(message.data, null, 2)}</pre>
                                    )}
                                    {message.event != undefined && (<pre>event: {message.event}</pre>)}
                                    {message.retry != undefined && (<pre>retry: {message.retry}</pre>)}
