...
export default function ConnectionsList({
  ...
  onConnectionSelected: (connection: Connection) => void
    ...
    <>
      {!data?.connections || data.connections.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <div className="text-lg mt-5 text-orange-500">
            No Connections Yet!
          </div>
          <div className="text-center mt-5">
            Configure this Sniffer as a Man-in-the-Middle Proxy for your MCP
            Server to sniff HTTP traffic.
          </div>

          <div className="text-center mt-5 font-bold">Setup Instructions:</div>
          <div className="text-center mt-2">
            1. Run your MCP Server to listen for connections on{' '}
            <a
              className="text-blue-500"
              href={`http://${data?.destination_ip}:${data?.destination_port}`}
            >
              http://{data?.destination_ip}:{data?.destination_port}
            </a>
          </div>
          <div className="text-center mt-2">
            2. Configure your MCP Client to connect to this MCP Sniffer on{' '}
            <a
              className="text-blue-500"
              href={`http://${data?.source_ip}:${data?.source_port}`}
            >
              http://{data?.source_ip}:{data?.source_port}
            </a>
          </div>
        </div>
      ) : (
                ...
                >
                  ...
                  <td className="text-left p-2 flex flex-row">
                    <div className="flex flex-row items-center">
                      <div
                        className={`w-2 h-2 m-1.5 rounded-full ${
                          conn.response.status_code >= 500
                            ? 'bg-red-500'
                            : conn.response.status_code >= 400
                              ? 'bg-orange-500'
                              : conn.response.status_code >= 300
                                ? 'bg-blue-500'
                                : 'bg-green-500'
                        }`}
                      ></div>
                  ...
                  <td className="text-left p-2">
                    <div className="flex flex-row items-center">
                      ...
                      ></div>
                      <div className="mx-0.5">
                        {conn.active == true ? 'Connected' : 'Closed'}
                      </div>
                    </div>
                  </td>
